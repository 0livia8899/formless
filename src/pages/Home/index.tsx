...
import gamebg from './images/bg.jpg';
...
export default function Home() {
  ...
  const [isIOSFillBg, setIsIOSFillBg] = useState<boolean>(false);
  ...
  const onOk = async () => {
    ...
    var val = { 
      "bankerAmount": bet_dict[0] * 1000000,
      "playerAmount": bet_dict[1] * 1000000,
      "tieAmount": bet_dict[2] * 1000000,
      "bankerPairAmount": bet_dict[3] * 1000000,
      "playerPairAmount": bet_dict[4] * 1000000
    }
    console.log(val);
    // check approve
    try{
    ...
    const handleMessage = async (event: any) => {
      ...
      if (event.data) {
        if (event.data.type == 'lottery') {
          ...
          setWithdrawVisible(true);
        }else if (event.data.type == 'balanceNotEnough'){
        ...
        }else if(event.data.type == 'iosChangeBg'){
          if(isIOS){
            setIsIOSFillBg(true)
          }
        }
  ...
  useEffect(() => {
    ...
    //   window.getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-left')
    //   + ' ' 
    //   window.getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-right')
    //   + ' ' 
    //   window.getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-top')
    //   + ' ' 
    //   window.getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-bottom');
    if(window.innerHeight == screen.height){
      setSafeAreaHeight('全屏');
    }else{
      setSafeAreaHeight('非全�?');
    }
  }, [window.innerHeight]);
  const handleClick = () => {
    setMessageShow("left:"+window.getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-left')+
  ...
  return (
    <div className="home">
      ...
      {false && <div className="safe-area-box1"></div>}
      {false && <div className="safe-area-box2"></div>}
      {false && <div className="safe-area-box3"></div>}
      {false && <div className="safe-area-box4"></div>}

       { false &&
         <div className="bottom-center-button">
          <label className='safe-area-label'>{safeAreaHeight}</label>
        </div>
      }
